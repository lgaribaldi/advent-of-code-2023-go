...
func day23(c chan string) {
	...
	for pos := range forestMap {
		for i := range dir {
			...
			if input[position.X][position.Y] == '.' || input[position.X][position.Y] == byte(dirSlope[i]) {
			walkUntilNextNode:
				for {
	...
	log.Printf("23A : %d %v", test, ok)
}

func longestPath(node image.Point, visited map[image.Point]struct{}, forestMap *map[image.Point]crosspath) (int, bool) {
	current := (*forestMap)[node]
	visited[node] = struct{}{}
	distance := 0
	for _, path := range current.paths {
		if _, ok := visited[path.dest]; ok {
			continue
		}
		newVisited := make(map[image.Point]struct{})
		for k := range visited {
			newVisited[k] = struct{}{}
		}
		if d, ok := longestPath(path.dest, newVisited, forestMap); ok && (d+path.length > distance) {
			distance = d + path.length
		}

	}
	if distance == 0 {
		return 0, current.end
	}
