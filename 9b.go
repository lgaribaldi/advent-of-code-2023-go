...
func day9b(c chan string) {
	...
	for j := 0; j < len(sequence); j++ {
		...
		prevValue := subSequence[len(subSequence)-2][0]
		for level := len(subSequence) - 3; level >= 0; level-- {
			prevValue = subSequence[level][0] - prevValue
		}
		log.Printf("Previous value for sequence %d", prevValue)
		total += prevValue
	}

	log.Printf("9B Total: %d", total)
}
